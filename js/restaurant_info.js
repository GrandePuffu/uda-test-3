...
/**
 * Create restaurant HTML and add it to the webpage, same from previous assignment except favorite
 */
...
*/
  ...
  element.removeAttribute('aria-label');
  element.setAttribute('aria-label','Unfavorite restaurant');
...
/*Avoiding trap Tap. Very useful tutorial: https://codepen.io/hidanielle/pen/MyggJJ */
trapTabKey=(e)=> {
  var focusableElements = modal.querySelectorAll(focusableElementsString);
  // Convert NodeList to Array
  focusableElements = Array.prototype.slice.call(focusableElements);
...
/*Avoiding empty fields in the form */
...
if(formInputs[0].value ==='' || (formInputs[1].value <1 || formInputs[1].value >5 || formInputs[1].value ==='') || formInputs[2].value ==='')
 {
   valid =false;
   message_container.innerHTML='The above field is empty';
...
/**
 * Separate Google Maps rendering according to size. Further optimization: merge with the function from main html file
 */
...
window.InitMobileMap=() => {
  ...
  mapImage.setAttribute('src',`https://maps.googleapis.com/maps/api/staticmap?center=${restaurant.latlng.lat},${restaurant.latlng.lng}&markers=size:large|color:red|${restaurant.latlng.lat},${restaurant.latlng.lng}&zoom=16&size=400x400&maptype=roadmap&key=AIzaSyC880A8PxMSvGATKoCm6j4wFn_JE-FnjQM`);
